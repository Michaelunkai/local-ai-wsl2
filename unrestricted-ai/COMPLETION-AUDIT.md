...
| --- | --- | --- |
| Same script sets up and opens the system | Exact `Setup-and-Open.ps1` run exited 0; final unchanged rerun opened the browser in 13.69 seconds and skipped builds | Verified on this host |
...
| Broad online research with sources | One final 26-check suite passed live search, WebUI search/embedding, JavaScript crawl/render and a sourced search-crawl-model answer | Verified for supported public sources; exhaustive coverage is impossible to prove |
...
| Automatic recovery | WSL anchor, systemd bridge watchdog and Windows supervisor all active; controlled fault tests passed, including public DNS and HTTPS after bridge repair | Verified current recovery paths |
...
- `data/indexer/verification.json` (26/26 passing in one post-repair run)
