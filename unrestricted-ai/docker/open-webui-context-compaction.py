...
- Keep similarly named or formatted values separate. A later file or tool result must not overwrite an earlier user-provided fact.
- If values differ, label the distinction explicitly so later answers use the value that matches the question.
...
SUMMARY_PROMPT_CHAR_LIMIT = 80000
SUMMARY_FALLBACK_CHAR_LIMIT = 6000
...
async def compact_messages_for_request(
    ...
    if not _exceeds_token_threshold(messages, system_prompt, previous_summary, token_threshold):
...
def _find_compaction_boundary(messages: list[dict], retention_percentage: int = 40) -> int:
    ...
    user_boundaries = [idx for idx, message in enumerate(messages) if message.get('role') == 'user']
    if len(messages) <= 3 and len(user_boundaries) >= 2:
        # A single tool-heavy assistant turn can fill the context before the
        # normal multi-turn boundary exists. Keep the new user request and
        # summarize everything before it.
        return user_boundaries[-1]
    keep_count = max(2, len(messages) * retention_percentage // 100)
    target = max(1, len(messages) - keep_count)
    boundaries = user_boundaries[1:]
    return next((idx for idx in reversed(boundaries) if idx <= target), user_boundaries[-1] if len(user_boundaries) >= 2 else 0)
...
async def _generate_summary(
    ...
    # Tool results and pasted files can be much larger than the model's
    # available context. Bound only the summarizer input; the original chat
    # remains intact in storage and is still available after compaction.
    prompt = _bounded_text(prompt, SUMMARY_PROMPT_CHAR_LIMIT)
    ...
    try:
        ...
        log.exception('Context summary generation failed; using a local bounded summary')
...
def _bounded_text(value: Any, limit: int) -> str:
    ...
    if len(value) <= limit:
        return value
    marker = '\n...[middle omitted for bounded context summary]...\n'
...
def _fallback_summary(previous_summary: str | None, compacted_messages: list[dict]) -> str:
    parts = []
    if previous_summary:
        parts.append('[previous summary]\n' + _bounded_text(previous_summary, 2000))
    for message in compacted_messages:
        content = get_content_from_message(message)
        if content:
            parts.append(f'- {message.get("role", "unknown")}: {_bounded_text(content, 1200)}')
        if message.get('output'):
            parts.append(f'- tool/result data: {_bounded_text(message["output"], 800)}')
    return _bounded_text('\n'.join(parts), SUMMARY_FALLBACK_CHAR_LIMIT)
