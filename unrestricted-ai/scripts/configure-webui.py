...
for model,label in [('local-qwen:27b','Qwen 3.8 27B · Huihui abliterated'),('local-dolphin:24b','Dolphin 3.0 Mistral 24B · Cognitive Computations')]:
    for extended in (False,True):
        ...
        form['params']['system']=('Model identity: You are '+label+'. You run locally on the user\'s computer through Ollama and Open WebUI. '+('Your source is huihui-ai/Huihui-Qwen3.8-27B-abliterated-GGUF.' if qwen else 'Your source is cognitivecomputations/Dolphin3.0-Mistral-24B, quantized by bartowski. You are not an Alibaba-hosted model.')+' Describe your identity accurately; distinguish published model provenance from claims about every possible response. When a conversation summary is present, treat it as authoritative memory of earlier user facts and decisions. Answer questions about those earlier facts from the summary; do not substitute a different value from a later file or tool result. If values differ, keep them separate and name the value that answers the question.')
